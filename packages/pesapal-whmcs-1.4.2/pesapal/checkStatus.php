...
class pesapalCheckStatus {
	...
	public function __construct(){
		...
		$this->gatewaymodule  		= "pesapal"; 
		$this->gateway        		= getGatewayVariables($this->gatewaymodule);	
		$consumer_key 			= $this->gateway['consumerkey'];
		$consumer_secret 		= $this->gateway['consumersecret'];
		...
		$this->QueryPaymentStatus 		= $gatewayapi.'/API/QueryPaymentStatus';
		$this->QueryPaymentStatusByMerchantRef	= $gatewayapi.'/API/QueryPaymentStatusByMerchantRef';
		$this->querypaymentdetails 		= $gatewayapi.'/API/querypaymentdetails';
	}
	
	function checkTransactionStatus($pesapalMerchantReference,$pesapalTrackingId = NULL){
		
		if($pesapalTrackingId)
			$statusapi = $this->QueryPaymentStatus;
		else
			$statusapi = $this->QueryPaymentStatusByMerchantRef;
		
		//get transaction status
		$request_status = OAuthRequest::from_consumer_and_token( $this->consumer, $this->token, "GET", $statusapi, $this->params);
		$request_status->set_parameter("pesapal_merchant_reference", $pesapalMerchantReference);
		if($pesapalTrackingId) $request_status->set_parameter("pesapal_transaction_tracking_id",$pesapalTrackingId);
	...
	function getTransactionDetails($pesapalMerchantReference,$pesapalTrackingId){
		...
		$request_status = OAuthRequest::from_consumer_and_token( $this->consumer, $this->token, "GET", $this->querypaymentdetails, $this->params);
		$request_status->set_parameter("pesapal_merchant_reference", $pesapalMerchantReference);
